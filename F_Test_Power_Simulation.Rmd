...
var1popl_list <- rep(5, length(n1_list))
var0popl_list <- rep(100, length(n1_list))
mu1 <- 0
mu0 <- 0 
...
result_table_phi005 <- simulate_power(var1popl_list, var0popl_list, var1_list, var0_list, n1_list, n0_list, alpha_list)
...
Plot sample size v.s. power for $\phi = 0.05$

```{r plot_samplesize_phi005}

phi005 <- ggplot(result_table_phi005, aes(x = n1, y = power)) + 
  geom_point() + 
...
### $\phi = 0.1$
```{r simulate_samplesize_phi01}


n1_list <- seq(10, 200, by = 1)
n0_list <- n1_list

var1popl_list <- rep(1, length(n1_list))
var0popl_list <- rep(10, length(n1_list))
mu1 <- 0
mu0 <- 0 
...
result_table_phi01 <- simulate_power(var1popl_list, var0popl_list, var1_list, var0_list, n1_list, n0_list, alpha_list)
...
Plot sample size v.s. power for $\phi = 0.1$

```{r plot_samplesize_phi01}

phi01 <- ggplot(result_table_phi01, aes(x = n1, y = power)) + 
  geom_point() + 
  scale_x_continuous(breaks = seq(5, 210, by = 10)) +
  scale_y_continuous(breaks = seq(0, 1, by = 0.05))
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
### $\phi = 0.3$
```{r simulate_samplesize_phi03}


n1_list <- seq(10, 200, by = 1)
n0_list <- n1_list

var1popl_list <- rep(3, length(n1_list))
var0popl_list <- rep(10, length(n1_list))
mu1 <- 0
mu0 <- 0 
...
result_table_phi03 <- simulate_power(var1popl_list, var0popl_list, var1_list, var0_list, n1_list, n0_list, alpha_list)
...
Plot sample size v.s. power for $\phi = 0.3$

```{r plot_samplesize_phi03}

phi03 <- ggplot(result_table_phi03, aes(x = n1, y = power)) + 
  geom_point() + 
  scale_x_continuous(breaks = seq(5, 210, by = 10)) +
  scale_y_continuous(breaks = seq(0, 1, by = 0.05))
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
result_table_phi05 <- simulate_power(var1popl_list, var0popl_list, var1_list, var0_list, n1_list, n0_list, alpha_list)
...
```{r plot_samplesize_phi05}

phi05 <- ggplot(result_table_phi05, aes(x = n1, y = power)) + 
  geom_point() + 
  scale_x_continuous(breaks = seq(5, 210, by = 10)) +
  scale_y_continuous(breaks = seq(0, 1, by = 0.05))
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
ggarrange(phi005, phi01, phi03, phi05,
          labels = c("phi = 0.05", "phi = 0.1", "phi = 0.3", "phi = 0.5"),
          label.x = 0.5,
          label.y = 0.5,
...
1. As $\phi$ increases from $0.05$ to $0.5$, the rate of change in power for small values of sample size n decreases. 
That is, for smaller $\phi$, there will be a large increase in power when increasing a small sample size n. However, for larger $\phi$, there will not be an increase in power as large as the one for smaller $\phi$ when increasing a small sample size n. 

2. On the other hand, as $\phi$ increases from $0.05$ to $0.5$, it requires larger sample size n for power to reach an "asymptote" (i.e. a point at which there is little increase in power when increasing sample size n). 
That is, for smaller $\phi$, smaller sample size n is needed for the power to reach an "asymptote" compared to the sample size n needed for larger $\phi$. 

3. The graph actually reminds me of a variogram in spatial statistics. 
