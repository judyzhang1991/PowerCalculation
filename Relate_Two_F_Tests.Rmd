...
Note: this is the formula for a two sample t test with alternative to be two sided: 

$n = \frac{(t_{\text{crit}} + t_{1-\beta})^2}{\delta_{a}^{2}}\frac{1}{P(1-P)}$,

where $n = n_{0} + n_{1}$ is the total sample size;
$P = \frac{n_{1}}{n}$ is the proportion of treatment units in the sample; 
$\delta_a = \frac{\mu_a}{\sigma} = \frac{\mu_1 - \mu_0}{\sigma}$ is the effect size (measured by the standardized mean difference),
where $\sigma^2$ is the shared population variance of the treatment group and the control group. 


Note: as $t_{\text{crit}}$ and $t_{1-\beta}$ are both functions of sample size $n$ (via d.f.), *for large sample sizes* we will use z-scores (quantiles for standardized normal distribution $N(0,1)$) to approximate them as for large sample sizes (i.e. large d.f.s), t distribution is very close to the standard normal distribution. That is, we will use $z_{\text{crit}}$ and $z_{1-\beta}$ instead. 
...
for(i in 1:length(z_ps)){
  ...
  for(j in 1:length(deltas)){
    ...
    ns_new[i,j] <- round(((z_crit + z_ps[i])^2)/(deltas[j]^2)*(1/(P*(1-P))))
...
meanTest_power50to80_sampleSizes_valid_new <- ns_new %>% pivot_longer(c("...1", "...2", "...3", "...4", "...5"), names_to = "Delta", values_to = "Total_Sample_Size") %>%
  mutate(
    Effect_Size = case_when(
      Delta == "...1" ~ 0.1,
      Delta == "...2" ~ 0.2,
    ...
    Sample_Size_per_Group = round(Total_Sample_Size / 2)
  ) %>%
  
  select (Power, Effect_Size, Total_Sample_Size, Sample_Size_per_Group)




meanTest_power50to80_sampleSizes_valid_new
...
ggplot(data = result_table_unique, mapping = aes(x = n, y = power, group = phi, color = phi)) + 
  geom_point() +
  geom_line(size = 0.5) +
  geom_hline(yintercept = 0.8, size = 1, linetype = "dashed", color = "#1B4F72") +
   geom_hline(yintercept = 0.7, size = 1, linetype = "dashed", color = "#1B4F72") +
   geom_hline(yintercept = 0.6, size = 1, linetype = "dashed", color = "#1B4F72") +
   geom_hline(yintercept = 0.5, size = 1, linetype = "dashed", color = "#1B4F72") +
  scale_x_continuous(name = "Sample Size (per group)", breaks = c(seq(0, 1600, 100), 64, 99, 175, 393, 1571)) +
  scale_y_continuous(name = "Power", breaks = sort(c(0.8, 0.7, 0.6, 0.5))) +
  scale_color_manual(name = "Population Variances Ratio", values=c("#B7950B", "#0BB795", "#B387C2", "#FFC300", "#F03A5E")) +
  ggtiltle("Sample Size Needed for Different Power Levels by Levels of Population Variance Ratio") + 
...
1. For $\phi = 1.1$ or $\phi = 1/1.1$, even with large sample sizes (which enables finding small SMD), it is hard to detect difference in variances. 
